import pandas as pd
import numpy as np
import os
import joblib

from xgboost import XGBRegressor
from sklearn.multioutput import MultiOutputRegressor
from sklearn.metrics import (
    mean_absolute_error,
    mean_squared_error,
    r2_score
)


# ============================================================
# CONFIGURATION
# ============================================================

INPUT_FILE = "data/features/karachi_features_v2.csv"

MODEL_DIR = "models"

MODEL_FILE = (
    f"{MODEL_DIR}/xgboost_72h.pkl"
)

RESULT_FILE = (
    f"{MODEL_DIR}/xgboost_72h_results.csv"
)


# ============================================================
# TARGETS
# ============================================================

TARGET_COLUMNS = [
    f"target_aqi_t_plus_{i}"
    for i in range(1, 73)
]


# ============================================================
# LOAD DATA
# ============================================================

print("=" * 60)
print("PEARLS AQI — XGBOOST 72-HOUR MODEL")
print("=" * 60)

df = pd.read_csv(
    INPUT_FILE,
    parse_dates=["timestamp"]
)

df = df.sort_values(
    "timestamp"
).reset_index(drop=True)

print(
    f"\nDataset rows: {len(df)}"
)

print(
    f"Dataset columns: {len(df.columns)}"
)


# ============================================================
# FEATURES / TARGETS
# ============================================================

excluded = (
    ["timestamp"]
    + TARGET_COLUMNS
)

feature_columns = [
    col
    for col in df.columns
    if col not in excluded
]

X = df[feature_columns]

Y = df[TARGET_COLUMNS]


print(
    f"\nFeature columns: {len(feature_columns)}"
)

print(
    f"Target columns: {len(TARGET_COLUMNS)}"
)


# ============================================================
# TIME SERIES SPLIT
# ============================================================

split_index = int(
    len(df) * 0.80
)

X_train = X.iloc[
    :split_index
]

X_test = X.iloc[
    split_index:
]

Y_train = Y.iloc[
    :split_index
]

Y_test = Y.iloc[
    split_index:
]


print("\n" + "=" * 60)
print("TIME-SERIES SPLIT")
print("=" * 60)

print(
    f"Training rows: {len(X_train)}"
)

print(
    f"Testing rows: {len(X_test)}"
)

print(
    "\nTraining period:"
)

print(
    f"{df['timestamp'].iloc[0]} → "
    f"{df['timestamp'].iloc[split_index - 1]}"
)

print(
    "\nTesting period:"
)

print(
    f"{df['timestamp'].iloc[split_index]} → "
    f"{df['timestamp'].iloc[-1]}"
)


# ============================================================
# XGBOOST MODEL
# ============================================================

print("\n" + "=" * 60)
print("TRAINING XGBOOST")
print("=" * 60)

base_model = XGBRegressor(
    n_estimators=300,
    max_depth=6,
    learning_rate=0.05,
    subsample=0.8,
    colsample_bytree=0.8,
    objective="reg:squarederror",
    random_state=42,
    n_jobs=-1
)

model = MultiOutputRegressor(
    base_model,
    n_jobs=-1
)

print(
    "\nTraining model..."
)

model.fit(
    X_train,
    Y_train
)

print(
    "Training complete."
)


# ============================================================
# PREDICTION
# ============================================================

print(
    "\nGenerating predictions..."
)

predictions = model.predict(
    X_test
)


# ============================================================
# EVALUATION
# ============================================================

print("\n" + "=" * 60)
print("72-HOUR PERFORMANCE")
print("=" * 60)


results = []

for i in range(72):

    actual = Y_test.iloc[:, i]

    predicted = predictions[:, i]

    mae = mean_absolute_error(
        actual,
        predicted
    )

    rmse = np.sqrt(
        mean_squared_error(
            actual,
            predicted
        )
    )

    r2 = r2_score(
        actual,
        predicted
    )

    results.append({
        "horizon_hours": i + 1,
        "MAE": mae,
        "RMSE": rmse,
        "R2": r2
    })


results_df = pd.DataFrame(
    results
)


# ============================================================
# SELECTED HORIZONS
# ============================================================

selected = results_df[
    results_df["horizon_hours"].isin([
        1,
        6,
        12,
        24,
        48,
        72
    ])
]

print(
    selected.to_string(
        index=False
    )
)


# ============================================================
# OVERALL PERFORMANCE
# ============================================================

print("\n" + "=" * 60)
print("OVERALL PERFORMANCE")
print("=" * 60)

print(
    f"Average MAE: "
    f"{results_df['MAE'].mean():.4f}"
)

print(
    f"Average RMSE: "
    f"{results_df['RMSE'].mean():.4f}"
)

print(
    f"Average R²: "
    f"{results_df['R2'].mean():.4f}"
)


# ============================================================
# SAVE MODEL
# ============================================================

os.makedirs(
    MODEL_DIR,
    exist_ok=True
)

joblib.dump(
    model,
    MODEL_FILE
)


# ============================================================
# SAVE RESULTS
# ============================================================

results_df.to_csv(
    RESULT_FILE,
    index=False
)


# ============================================================
# SAVE FEATURES
# ============================================================

pd.Series(
    feature_columns
).to_csv(
    f"{MODEL_DIR}/xgboost_feature_columns.csv",
    index=False,
    header=["feature"]
)


# ============================================================
# COMPLETE
# ============================================================

print("\n" + "=" * 60)
print("XGBOOST TRAINING COMPLETE")
print("=" * 60)

print(
    f"\nModel saved to:"
)

print(
    MODEL_FILE
)

print(
    "\nResults saved to:"
)

print(
    RESULT_FILE
)